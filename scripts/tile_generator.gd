...
const chunknode = preload("res://scenes/tile.tscn")
...
export (float) var chunk_size_v = 64
export (float) var chunk_size_h = 32
...
func _get_player_chunk(pos):
	...
	chunk_pos.y = int(pos.y/chunk_size_v)
	chunk_pos.x = int(pos.x/chunk_size_h)
	if pos.x < 0:
		chunk_pos.x -= 1
	if pos.y < 0:
		chunk_pos.y -= 1
...
# warning-ignore:unused_variable
			...
			if active_coord.find(chunk_coords) == -1:
				...
				chunk.position.x = chunk_coords.x * chunk_size_h
				chunk.position.y = chunk_coords.y * chunk_size_v
	...
	for x in deleting_chunks:
		...
		active_chunks[index].delete_chunk()
		#print(GlobalGen.retrive_chunk(active_chunks[index].chunk_coords))
